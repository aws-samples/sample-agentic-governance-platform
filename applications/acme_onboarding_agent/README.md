...
> **Persona — Insurance Onboarding Support.** This agent helps **new policyholders** get
> set up after buying an Acme policy. It is encouraging, patient, and jargon-free:
> walking customers through onboarding steps, required documents, portal/account setup, and
> a plain-language explanation of what their new policy covers and how to get help. It
> **explains coverage in plain terms but does not give fiduciary advice or recommend
> coverage changes**, escalates underwriting/eligibility/pricing questions, and makes no
> binding commitments. The persona lives in the `SYSTEM_PROMPT` constant in `agent.py`.
...
> land in Langfuse (onboarding conversations — customer names, documents, and policy details).
...
1. **Configure and launch** the runtime (writes `.bedrock_agentcore.yaml`, builds and pushes
   ...
   ```bash
   ./deploy.sh
   # equivalently: agentcore configure --entrypoint agent.py --name acme_onboarding_agent && agentcore launch
   ```
