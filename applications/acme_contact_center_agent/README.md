...
> **Persona — Contact Center.** This agent is the warm, efficient virtual front desk for
> Acme customers: it triages a broad mix of servicing enquiries (policy and coverage
> basics, billing, document requests, claim status) and routes customers to the right
> specialist (claims, FNOL, sales, underwriting). It makes no coverage, pricing, or claim
> commitments and verifies identity scope before discussing account specifics. The persona
> lives in the `SYSTEM_PROMPT` constant in `agent.py`.
...
> land in Langfuse (customer servicing text). For the prototype this targets a **non-prod**
> Langfuse project with restricted access + short retention. **Before production**, add a
> redaction layer (OTEL Collector or source-level) — the Langfuse SDK `mask=` does **not** cover
> Strands' raw OTLP spans.
...
1. **Configure and launch** the runtime (writes `.bedrock_agentcore.yaml`, builds and pushes
   ...
   ```bash
   ./deploy.sh
   # equivalently: agentcore configure --entrypoint agent.py --name acme_contact_center_agent && agentcore launch
   ```
