...
#
# LANGFUSE OBSERVABILITY (E26/T10): the reference agent now emits Strands traces to Langfuse for
# parity with the acme_* agents. Its project key is read from AWS Secrets Manager by agent.py at
# import (NEVER a hardcoded literal) — given the secret NAME + host as runtime env. The two vars
# below are NON-SECRET (a URL + a Secrets Manager NAME; the {public_key,secret_key} VALUE stays in
# Secrets Manager and NEVER transits this script), operator-supplied via the environment. When
# either is empty, telemetry is simply disabled (the agent still runs). DISABLE_ADOT_OBSERVABILITY
# defeats the AgentCore ADOT set-once trap so the Langfuse exporter wins the global TracerProvider.
# All three are passed to the runtime on `agentcore launch --env` (first deploy) AND force-injected
# into the merged env on a redeploy (survive launch's full-replace, alongside the backend MCP env).
...
AGENT_NAME="${AGENT_NAME:-acme_reference_agent}"
...
#
# LANGFUSE (E26/T10): pass DISABLE_ADOT_OBSERVABILITY + the NON-SECRET LANGFUSE_HOST /
# LANGFUSE_SECRET_NAME on the runtime via the toolkit's native --env flag so the Langfuse OTEL
# exporter (wired in agent.py from the Secrets-Manager key) wins the global TracerProvider. No
# secret VALUE transits this script. On a redeploy, launch's full-replace would wipe these, so
# step 2b re-asserts them in the merged env.
...
# the jq below). Idempotent: if launch left BOTH the authorizer AND the env untouched, skip the
# restore. A single UpdateAgentRuntime restores both at once.
if [ "${RUNTIME_EXISTS}" = "1" ] && [ "${CAPTURE_OK}" = "1" ]; then
  ...
  if aws bedrock-agentcore-control get-agent-runtime \
    ...
    # LANGFUSE (E26/T10): what are DISABLE_ADOT_OBSERVABILITY / LANGFUSE_HOST / LANGFUSE_SECRET_NAME
    # on the runtime AFTER launch? If launch's --env took, they are already set; if not, the restore
    ...
    # LANGFUSE (E26/T10): assert the observability env unless already correct (idempotency guard —
    # empty env ⇒ never asserted, telemetry stays disabled and the runtime is left as-is).
    ...
    if [ "${AUTH_NEEDS_RESTORE}" = "0" ] && [ "${ENV_NEEDS_RESTORE}" = "0" ] && [ "${ADOT_NEEDS_SET}" = "0" ] && [ "${LANGFUSE_NEEDS_SET}" = "0" ]; then
      ...
      #     clobbered (`(post // {}) * (pre // {})` is a recursive merge, right operand wins);
      ...
      if jq -n \
            ...
            | {
                ...
                environmentVariables:  (
                  ...
                  # LANGFUSE (E26/T10): force-inject the observability env LAST, when configured, so
                  # it survives a launch full-replace on a redeploy. Distinct keys — never collide
                  # with the governance/MCP env. No secret VALUE is ever placed here.
