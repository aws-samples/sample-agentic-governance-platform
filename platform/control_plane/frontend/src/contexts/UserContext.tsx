...
interface UserInfo {
  ...
  can_deploy: boolean;
  oid: string | null;
  // Human display name from the Entra `name` claim; null in dev-auth — the
  // sidebar footer falls back to the email alias.
  name: string | null;
  // The caller's resolved tenant memberships (E24/T4). The backend degrades to
  // [] on a failed resolve, so this is always an array.
  tenants: UserTenant[];
}
