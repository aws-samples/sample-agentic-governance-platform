...
resource "aws_cloudfront_distribution" "main" {
  ...
  viewer_certificate {
    ...
    # AWS PINS this to TLSv1 whenever the default *.cloudfront.net certificate is used and
    # silently ignores anything stronger, so hardcoding TLSv1.2_2021 made every subsequent
    # `terraform plan` report a 2-resource in-place update that could never converge. Only a
    # custom ACM cert can carry the modern policy — which is exactly when we ask for it.
    minimum_protocol_version = var.domain_name != "" && var.hosted_zone_id != "" ? "TLSv1.2_2021" : "TLSv1"
  }
