  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        },
        {
          ...
          value = local.enable_sso ? "AGP Login" : ""
        },
        {
...
resource "aws_ecs_task_definition" "clickhouse" {
  ...
  volume {
    ...
    efs_volume_configuration {
      ...
      root_directory = "/"
      # E28D security pass: encrypt the ClickHouse data mount in transit (CKV_AWS_97).
      # ECS mounts via stunnel on 127.0.0.1:20049 -> EFS :2049; the EFS SG already
      # admits 2049 from the task SG, so no SG change is needed.
...
resource "aws_ecs_service" "clickhouse" {
  ...
  # STOP THE OLD TASK BEFORE STARTING THE NEW ONE. ClickHouse takes an exclusive PID lock on
  # its data directory (the `status` file in /var/lib/clickhouse), and this is a SINGLETON
  # backed by ONE EFS volume — so two tasks can never coexist: the second exits 76 immediately.
  # On the ECS defaults (min 100 / max 200) a task-definition change is therefore UNDEPLOYABLE,
  # not merely slow: ECS must place the replacement before draining the old task, the
  # replacement always loses the lock, and the service crash-loops a new task every ~60s
  # forever while the old revision keeps serving. Found live while bumping the image — it was latent
  # until then only because this task def had never been replaced.
  # 0/100 makes the real constraint explicit and costs a short gap in trace ingestion, which is
  # the only honest option for a singleton holding an exclusive lock. Do NOT "restore"
  # zero-downtime here without first giving ClickHouse a replicated, multi-volume topology.
  # AZ Rebalancing must be DISABLED to express this: ECS rejects maximumPercent <= 100 while it
  # is enabled (InvalidParameterException on UpdateService). Rebalancing exists to spread
  # MULTIPLE tasks across AZs; on a single task pinned to one EFS volume it has nothing to
  # balance, so turning it off costs nothing and is what makes 0/100 expressible.
