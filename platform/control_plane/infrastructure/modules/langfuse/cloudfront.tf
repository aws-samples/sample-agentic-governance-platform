...
resource "aws_cloudfront_distribution" "langfuse" {
  ...
  viewer_certificate {
    ...
    # TLSv1 is the ONLY value AWS accepts with the default *.cloudfront.net certificate; it
    # pins the distribution there regardless of what we ask for. Requesting TLSv1.2_2021 made
    # every later `terraform plan` show a permanent, non-converging in-place update. This
    # distribution has no custom-domain path, so there is nothing to make conditional.
...
# Lambda@Edge is us-east-1-only (see lambda_edge.tf).
