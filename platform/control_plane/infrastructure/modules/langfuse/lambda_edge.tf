...
# Lambda@Edge: must be created in us-east-1 or CloudFront rejects the
# association with InvalidLambdaFunctionAssociation. The IAM role above is
# global, so it stays on the default provider.
