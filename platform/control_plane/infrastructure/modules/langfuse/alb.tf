...
resource "aws_lb_target_group" "langfuse" {
  # var.name already ends in "-langfuse"; a "-tg" suffix keeps the name unique
  # and under the 32-char target-group limit under the base "<project>-cp-<env>-<acct6>-langfuse" prefix.
  name        = "${var.name}-tg"
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = data.aws_vpc.vpc.id
