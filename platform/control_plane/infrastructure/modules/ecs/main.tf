  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        # Scoped to the AgentCore runtime default service-role name pattern; the
        # PassedToService condition ensures the role can only be passed to AgentCore.
        Resource = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/service-role/AmazonBedrockAgentCoreRuntime*"
        Condition = {
          StringEquals = {
