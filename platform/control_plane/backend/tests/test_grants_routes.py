...
def test_reprovision_schedules_langfuse_hook(entra_settings):
    """Reprovision is the RECOVERY affordance, so it must resume EVERY registration leg —
    including the Langfuse one. Found live 2026-08-14: an agent whose first registration
    failed at identity (the register-time hook behind it is starved — starlette background
    tasks stop at the first raising task) and was then recovered via /reprovision ended up
    provisioned but permanently without a Langfuse project. Every leg of the hook is
    idempotent, so scheduling it on an already-converged agent is a no-op."""
    agent = _make_agent(identity_status="failed")
    mock_registry = MagicMock()
    mock_registry.get.return_value = agent
    mock_registry.persist_identity.side_effect = lambda a: a
    ...
    mock_langfuse_svc = MagicMock()
    agents_module._langfuse_svc = mock_langfuse_svc
    try:
        with (
            patch.object(agents_module, "provision_langfuse_best_effort") as mock_hook,
            patch(
        ...
        assert resp.status_code == 202
        # TestClient runs background tasks after the response: identity provision first
        # (same order as the register route — the env-merge in the Langfuse leg must not
        # race the authorizer full-replace), then the Langfuse hook with the singleton.
        mock_identity.provision.assert_awaited_once()
        mock_hook.assert_called_once_with(agent, mock_langfuse_svc)
    finally:
        agents_module._langfuse_svc = None
