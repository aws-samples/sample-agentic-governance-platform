...
class GuardrailConfig(BaseModel):
    """The policy shape of a guardrail, with no tenant dimension.

    Split out from `GuardrailTemplateCreate` (E34/T5) because a PRESET is a catalog
    entry, not a create payload: `GuardrailPreset.config` must stay constructible
    without a tenant, while `tenant_id` is required to actually create a template.
    """
    name: str = Field(..., min_length=1, max_length=100)
...
class GuardrailTemplateCreate(GuardrailConfig):
    """Create payload. `tenant_id` is REQUIRED on new templates (E24 spec §6), the
    same rule `AgentCreate` and `McpServerCreate` already carry."""
...
class GuardrailTemplate(BaseModel):
    ...
    # Multi-tenancy (Epic 24, retrofitted in E34/T5) — the owning tenant.
    # Optional here (pre-E34 records hydrate as None); REQUIRED on GuardrailTemplateCreate.
